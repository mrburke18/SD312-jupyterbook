...
These approaches solve many of the issues we described for Tabular Q-Learning.  Our state space can now be continuous, or as large as we want, and we're learning generalizable facts about states, since every state is processed with the same function.  Noteably, the action space must still be small and discrete, as we have a finite number of nodes in the output layer.
