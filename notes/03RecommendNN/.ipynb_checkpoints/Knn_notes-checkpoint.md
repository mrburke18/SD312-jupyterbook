...
recommendations are full of Pokemon things, because my 9 and 10-year-olds just are having a resurgence of interest as they made new friends at school who are into Pokemon, and that's what they've used my profile to shop for recently).
...
Pokemon").  Each user then is given a fingerprint, or a list of which
...
pokemon-based stuff that I really don't want to see).
...
description of the item itself, which can be difficult to keep up with if you
have a large set of often-changing elements.  The main benefit of content-based
filtering is that you often need much less data to produce a useful
recommendation, because you can start with meaningful, human-designed features.

Of course, we could also imagine deploying systems that use both
content-based and collaborative approaches, which is what many are.
...
Suppose we have a bunch of ratings for movies, organized in a matrix which is "number of users" x "number of movies".  Given that somebody just
...
Your choice of "distance" is an important consideration.
...
Also, you might first choose to **perform PCA**.  (We will cover this next.) 
If your data actually lies in a lower dimension, you might as well do your measures 
of distance in that lower dimension, making it cheaper to compute.
